...
export default class ReactResumableJs extends React.Component {
    constructor(props) {
        ...
        this.state = {
            progressBar: 0,
            messageStatus: 'Please add some files',
            fileList: {files: []},
            isPaused: false,
            isUploading: false
        };
    ...
    componentDidMount = () => {
        ...
        ResumableField.on('fileSuccess', (file, fileServer) => {
            ...
            // console.log('asdas', fileServer.message, typeof(fileServer))

            const msg = JSON.parse(fileServer);


            this.setState({
                fileList: {files: currentFiles},
                messageStatus:  msg.success ? msg.message : msg.error
            }, () => {
        ...
        let markup = this.state.fileList.files.map((file, index) => {
            ...
            if (file.file.type.indexOf('video') > -1) {
                ...
                return <li className="thumbnail" key={uniqID}>
                    ...
                    <span onClick={(event) => this.removeFile(event, file, index)} href="#">[X]</span>
                </li>;
            }
    ...
    render() {
        ...
        return (
            <div id={this.props.dropTargetID} ref={node => this.dropZone = node}>
                ...
                <p>
                    Current Status: {this.state.messageStatus}
                </p>
                <br />
            </div>
        );
